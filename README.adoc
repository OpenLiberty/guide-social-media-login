...
[hotspot=logout file=2]`logout()` method in `HttpServletRequest`.
...
Before using `HttpServletRequest` [hotspot=logout file=2]`logout()` method to log the user out of the application,
the `LogoutHandler` [hotspot=getLogout hotspot=inject file=2]`getLogout()` method gets an implementation of `ILogout`
for GitHub and revokes OAuth2 permissions granted to the application by the user using the `ILogout`
[hotspot=revoke file=2]`logout()` method.

The response returned by the `ILogout` [hotspot=revoke file=2]`logout()` method is verified by checking if it has a
[hotspot=errorHandle file=2]`2xx` series status code.

The `LogoutHandler` [hotspot=getLogout file=2]`getLogout()` method will [hotspot=switch file=1]`return` the
[hotspot=inject file=1]`implementation` of `ILogout` based on the name of the social media provider used
by a user which is retrieved using the [hotspot=socialMediaName file=1]`UserProfileManager`.
...
GitHub's REST API provides a `DELETE` [hotspot=unauthorizeUrl file=0]`endpoint` that is used to revoke permissions from
the user for the application. The JAX-RS [hotspot=delete file=0]`HttpClient` is used to send a request to this endpoint.
...
If more than one social media login provider is added, a selection form is presented to the user,
where all the possible social media providers configured in your application are shown for the user to pick from.

Create the `<facebookLogin />` configuration element in `server.xml` to setup login for Facebook.
