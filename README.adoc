...
[role='command']
include::{common-includes}/devmode-mvnw-start-cd.adoc[]
...
The [hotspot=socialLogin file=0]`socialLogin` feature definition enables the Social Media Login feature in the application so that you can use the [hotspot=githubLogin file=0]`githubLogin` configuration element to configure GitHub as a social media login provider.
...
[role='no_copy']
----
Exception thrown by application class 'io.openliberty.guides.sociallogin.LogoutServlet.doPost:49'
java.lang.NullPointerException: Cannot invoke "jakarta.ws.rs.core.Response.getStatusInfo()" because "logoutResponse" is null
at io.openliberty.guides.sociallogin.LogoutServlet.doPost(LogoutServlet.java:49)
...
The [hotspot file=0]`LogoutServlet` provided in the `start` directory uses the [hotspot=logout file=0]`logout()` method in the `HttpServletRequest` class to log the user out of the application. After a user is logged out, the user is redirected to the [hotspot=redirect file=0]`hello.html` page.
...
You are also provided with a [hotspot file=1]`LogoutHandler` class and [hotspot file=2]`ILogout` interface to add custom logout logic that revokes the application permissions from a user's account.


The [hotspot=getLogout hotspot=inject file=0]`logoutHandler.getLogout()` method gets an implementation of the `ILogout` interface for GitHub and uses the [hotspot=revoke file=0]`logout()` method of that interface to revoke the OAuth2 permissions that are granted to the application by the user.

The response that's returned by this [hotspot=revoke file=0]`logout()` method is verified by the application, which checks whether it has a [hotspot=errorHandle file=0]`2xx` series status code.

The [hotspot=logout file=0]`request.logout()` method is then called to log the user out of the application.

The [hotspot=getLogout file=0]`logoutHandler.getLogout()` method returns the implementation of the [hotspot=inject file=1]`ILogout` interface based on the name of the social media provider that a user chooses. The social media provider's name is retrieved by using the [hotspot=socialMediaName file=1]`UserProfileManager` class. The `UserProfileManager` class returns the ID of the social media login configuration. In this application, when the application user selects GitHub, the name of the social media provider is [hotspot=githubLoginName file=1]`githubLogin`.

Implement the [hotspot file=2]`ILogout` interface to revoke permissions for the application from the user's GitHub account.
...
GitHub's REST API provides a `DELETE` endpoint, which is stored in the [hotspot=unauthorizeUrl file=3]`unauthorizeUrl` variable. The  `unauthorizeUrl` variable is used to revoke application permissions from the user. The JAX-RS [hotspot=delete file=3]`Client` is used to send a request to this `DELETE` endpoint.

Your GitHub application client ID and client secret credentials are obtained from the `github.client.id` and `github.client.secret` configuration properties. These credentials are encoded and stored in the [hotspot=encodeAuth file=3]`encodedAuth` variable and added to the request as an [hotspot=authHeader file=3]`Authorization` header.

The GitHub access token for a user, which is stored in the [hotspot=accessToken file=3]`accessToken` variable, is retrieved from their user profile that is modelled by the `UserProfile` class. The `UserProfile` object can be retrieved with the `UserProfileManager` class. The access token is used in the `DELETE` request body, which is stored in the [hotspot=createAccessTokenBody hotspot=addBody file=3]`requestBody` variable.
