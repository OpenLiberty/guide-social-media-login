...
of `ILogout` for GitHub&reg; and uses the [hotspot=revoke file=2]`logout()` method of the `ILogout` interface to revoke
...
Implement [hotspot file=3]`ILogout` for revoking permissions for the application from the user's GitHub&reg; account.
...
GitHub&reg;'s REST API provides a `DELETE` [hotspot=unauthorizeUrl file=0]`endpoint` that is used to revoke permissions
...
The GitHub&reg; [hotspot=accessToken file=0]`access token` that is retrieved from the user's `UserProfile` through the
