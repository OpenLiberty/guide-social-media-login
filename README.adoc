...
[hotspot=clientIdProperty file=0]`github.client.id` and [hotspot=clientSecretProperty file=0]`github.client.secret` by
Microprofile Config.
...
2. An [hotspot=sslConfig]`<ssl id="defaultSSLRef .../>` configuration element for configuring the SSL configuration that is used to connect to the social media
...
The [hotspot=30]`trustDefaultCerts` attribute of the [hotspot=sslConfig]`<ssl id="defaultSSLRef" .../>`
...
The client ID and client secret for your GitHub OAuth2 application are injected into the `<githubLogin />` configuration
element with the values of `github.client.id` and `github.client.secret` by Microprofile Config.
These values are supplied using the environment variables `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET`.
...
If more than one is added, there’s a selection form presented to the user, where all the possible social media providers
configured in your application are shown for the user to pick from. As an exercise, configure a second social media
provider, like Facebook, for social media login for your application.

You can use the `<facebookLogin />` configuration element to setup login for Facebook. The following links to Facebook
documentation will provide any additional information about setting up an OAuth2 application.
