...
GitHub's REST API provides a `DELETE` endpoint, which is stored in the [hotspot=unauthorizeUrl file=3]`unauthorizeUrl` variable. The  `unauthorizeUrl` variable is used to revoke application permissions from the user. The JAX-RS [hotspot=delete file=0]`Client` is used to send a request to this `DELETE` endpoint.
