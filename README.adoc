...
---- 
#Replace this `HelloService` class.#
...
- [hotspot=httpServletRequestContext]`javax.ws.rs.core.Context`
...
The [hotspot=httpServletRequestContext hotspot=httpServletRequestImport]`HttpServletRequest` context, which is injected using the
[hotspot=]contextAnnotation`@Config` annotation, allows you to get information about the logged in user
using [hotspot=userPrincipal hotspot=userPrincipalImport]`request.getUserPrincipal()`.
