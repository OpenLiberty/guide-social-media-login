...
1. Replace `<website URL` with each URL mentioned above and run the following command in a terminal window:
...
2. In the command output, the contents of each certificate looks like:
...
3. Copy each certificate from the chain and add it to a separate `.pem` file.
