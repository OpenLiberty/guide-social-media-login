...
google_cert.sh
[source, sh, linenums, role='code_column hide_tags=copyright'] 
---- 
include::finish/src/main/liberty/config/resources/security/scripts/google_cert.sh[]
----

Run the [hotspot file=0]`google_cert.sh` script under the `scripts` folder:
[role='command']
```sh
./scripts/google_cert.sh
```

This script uses [hotspot=certificate]`keytool` to download the TLS server public keys required for Google from the following domains:

- `accounts.google.com` (saved as `google.pem`)
- `www.googleapis.com` (saved as `googleapis.pem`)

It also adds the public keys into a truststore, [hotspot=certificate]`slts.p12` that you will configure your application to use for SSL later in the guide.
...
[source, sh, linenums, role='code_column hide_tags=copyright'] 
---- 
include::finish/src/main/liberty/config/resources/security/scripts/github_cert.sh[]
----

Run the [hotspot file=0]`github_cert.sh` script under the `scripts` folder:
[role='command']
```sh
./scripts/github_cert.sh
```

This script will download the TLS server public keys required for GitHub from the following domains:

- `www.github.com` (saved as `github.pem`)
- `api.github.com` (saved as `github_api.pem`)

It also adds the public keys into a truststore, `slts.p12` that you will configure your application to use for
SSL later in the guide.
...
[source, sh, linenums, role='code_column hide_tags=copyright'] 
---- 
include::finish/src/main/liberty/config/resources/security/scripts/amazon_cert.sh[]
----

Run the [hotspot file=0]`amazon_cert.sh` script under the `scripts` folder:
[role='command']
```sh
./scripts/amazon_cert.sh
```

This script will download the TLS server public keys required for Amazon from the following domains:
