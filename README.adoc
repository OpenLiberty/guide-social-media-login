...
Add the required [hotspot=newImports]`imports` to the `HelloService` class.
Add the [hotspot=rolesAllowed hotspot=rolesAllowedImport]`@RolesAllowed({"users"})` annotation.
Add the [hotspot=httpServletRequestContext hotspot=httpServletRequestImport]`HttpServletRequest` context.
Replace the `return "Hello, friend!";` to [hotspot=userPrincipal] `output user information`.
...
The [hotspot=rolesAllowed hotspot=rolesAllowedImport]`@RolesAllowed({"users"})` annotation restricts access to the service to users who are
in the `users` role.

The [hotspot=httpServletRequestContext hotspot=httpServletRequestImport]`HttpServletRequest` context allows you to use 
[hotspot=userPrincipal hotspot=userPrincipalImport]`request.getUserPrincipal()` to get information about the logged in user instead of outputting `friend`.
