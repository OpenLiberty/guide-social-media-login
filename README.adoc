...
Add the [hotspot=httpServletRequestContext]`HttpServletRequest` context.
Then, use [hotspot=userPrincipal]`request.getUserPrincipal()` to get information about the logged in user.
