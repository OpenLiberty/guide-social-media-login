...
Add OpenLiberty features to the [hotspot=features]`feature elements`

To enable https://openliberty.io/docs/ref/feature/#socialLogin-1.0.html[Social Media Login feature^] and other required features, add [hotspot=newFeatures]`feature elements` to the [hotspot=featureManager]`featureManager` in
`src/main/liberty/config/server.xml`
